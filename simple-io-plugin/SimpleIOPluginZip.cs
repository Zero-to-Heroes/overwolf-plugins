...
{
    public class SimpleIOPluginZip : IDisposable
    {
        IntPtr _window = IntPtr.Zero;

        public SimpleIOPluginZip(int window)
        {
        ...
        #region Events
        public event Action<object, object, object, object> onFileListenerChanged;
        ...
        public void zipAppLogFolder(string appName, Action<object, object> callback)
        {
            ...
            var directoryInfo = Directory.CreateDirectory(tmpDir);
        ...
        private void OnFileChanged(object id, object status, object isOldLine, object data)
        {
            if (onFileListenerChanged != null)
            {
                onFileListenerChanged(id, status, isOldLine, data);
            }
        }
